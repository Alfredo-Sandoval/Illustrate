...
from illustrate_web.api.deps import RateLimitExceeded, enforce_rate_limit, rate_limit_client_host, store_upload_file
...
async def upload_pdb(request: Request, file: UploadFile = File(...)) -> UploadResponse:
    try:
        enforce_rate_limit("upload", client_host=rate_limit_client_host(request))
